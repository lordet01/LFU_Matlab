% /*********************************************
% 	GIST pop-denoiser Module  (Ver.0.2m)
...
%          n_flag_R = 1;
    ...
    if n_flag == 0 %|| ~(sum(abs(s_frame_rect_2d)) > 0)
        ...
        LPorder_rec_R = prm.LPorder_recon;

        
        [LP_rec_b, var_L] = lpc(s_frame_bp_2d(1:size_fshift), LPorder_rec_L);
        [LP_rec_f, var_R] = lpc(s_frame_bp(size_fshift+1+200:size_frame), LPorder_rec_R);
        ...
        s_proc_bp_L = sqrt(var_L) * filter(1, LP_rec_b, res_noise);
        s_proc_bp_R = sqrt(var_R) * filter(1, LP_rec_f, res_noise);

        %s_proc_bp = s_proc_bp_L;
        s_proc_bp_L = s_proc_bp_L .* w2(size_frame+1:2*size_frame);
        s_proc_bp_R = s_proc_bp_R .* w2(1:size_frame);
        
        s_proc_bp = (s_proc_bp_L + s_proc_bp_R);
        
%         if n_flag_L == 1 && n_flag_R == 1
%             s_proc_bp = [s_frame_rect_1d(1:recon_L_L) ; s_proc_bp(recon_L_L+1:recon_R_R) ; s_frame_rect_1d(recon_R_R+1:size_frame)];
%         elseif n_flag_L == 1 && n_flag_R == 0
%             s_proc_bp = [s_frame_rect_1d(1:recon_L_L) ; s_proc_bp(recon_L_L+1:recon_L_R) ; s_frame_rect_1d(recon_L_R+1:size_frame)];
%         elseif  n_flag_L == 0 && n_flag_R == 1
%             s_proc_bp = [s_frame_rect_1d(1:recon_R_L) ; s_proc_bp(recon_R_L+1:recon_R_R) ; s_frame_rect_1d(recon_R_R+1:size_frame)];
%         end
        
%         n = 5; %BPF tap
%         Wn = [BP_cutL BP_cutH]; %HPF cut-off
%         s_proc_bp=train_bpf(s_proc_bp, fs, n, Wn);        
%         s_frame_rect_2d = train_bpf(s_frame_rect_2d, fs, n, Wn);    
%         s_frame_rect = train_bpf(s_frame_rect, fs, n, Wn);    
        
        %LR Power matching
        mean_b = mean(abs(s_frame_bp_2d(1 : size_fshift)));
        mean_f = mean(abs(s_frame_bp(size_fshift+1 : size_frame)));
        mean_c = mean(abs(s_proc_bp)) * prm.RECON_POWER;
        
        tilt = (mean_f - mean_b) / (size_frame-1);
        mean_curve = (1:size_frame)';
        mean_curve = (mean_curve -1) .* tilt + mean_b; 
        s_proc_bp = s_proc_bp .* mean_curve / mean_c;
...
%         s_proc_bp = zeros(size_frame,1);
%         s_proc_bp_L = zeros(size_frame,1);
%         s_proc_bp_R = zeros(size_frame,1);
...
%                 s_proc_bp_L(j) = s_proc_bp_L(j) -1 * LP_rec_b(lp) * s_frame_buff_L(size_frame+j-(lp-1));
%                 
%                 %Right to left reconstruction
%                 s_proc_bp_R(size_frame-j+1) = s_proc_bp_R(size_frame-j+1) -1 * LP_rec_f(lp) * s_frame_buff_R(2*size_frame-j+(lp-1)+1);
%             end
%             s_frame_buff_L(j+size_frame) = s_proc_bp_L(j);
%             s_frame_buff_R(2*size_frame-j+1) = s_proc_bp_R(size_frame-j+1);
%         end
%         s_proc_bp = (s_proc_bp_L + s_proc_bp_R) * 0.5;
...
%         s_proc_bp = [waveform_L; waveform_R];%./ window_scale;
