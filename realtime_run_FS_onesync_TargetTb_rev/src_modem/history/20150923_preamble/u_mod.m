...
[obj]=dsp_play(0, fs, Tb*31, 'start', 0);
...
while (1)
    % Add preamble
    if sync_on < 1
        [y, b_prev frame_idx]=encode_DPSK(header_bits, b_prev, frame_idx, fbit_bc, w, Tb, fs, fc, AMP);
        ...
        fwrite(fout, y, 'int16');
        if PLAY_PACKETWISE
            [obj]=dsp_play(obj, fs, Tb*31, 'record', y);
            disp('--send preamble--');
            [obj]=dsp_play(obj, fs, Tb*31, 'stop', y);
            [obj]=dsp_play(0, fs, Tb*46, 'start', 0);
        end
        
        %Save preamble sequences
        s_p = y;
        save(['src_modem/preamble/p_',fname_snd,'.mat'],'s_p');
    % Encode Data
    else
    ...
    if blk_cnt == N
        ...
        fwrite(fout, y, 'int16');
        if PLAY_PACKETWISE
            [obj]=dsp_play(obj, fs, Tb*46, 'record', y);
            disp('--send packet--');
        end
